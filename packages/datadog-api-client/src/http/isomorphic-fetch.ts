...
export class IsomorphicFetchHttpLibrary implements HttpLibrary {
  ...
  public shouldRetry(enableRetry:boolean, currentAttempt:number, maxRetries:number, responseCode:number):boolean{
    return (responseCode === 429 || responseCode >= 500 ) && maxRetries > currentAttempt && enableRetry
  }

  public calculateRetryInterval(currentAttempt:number, backoffBase:number, backoffMultiplier:number, headers: {[name: string]: string}) : number{

"use strict";
/**
 * Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
 * This product includes software developed at Datadog (https://www.datadoghq.com/).
 * Copyright 2020-Present Datadog, Inc.
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.Log = void 0;
var ObjectSerializer_1 = require("./ObjectSerializer");
/**
* Object describing a log after being processed and stored by Datadog.
*/
var Log = /** @class */ (function () {
    function Log() {
    }
    Log.getAttributeTypeMap = function () {
        return Log.attributeTypeMap;
    };
    Log.deserialize = function (data) {
        var res = new Log();
        res.content = ObjectSerializer_1.ObjectSerializer.deserialize(data.content, "LogContent", "");
        res.id = ObjectSerializer_1.ObjectSerializer.deserialize(data.id, "string", "");
        return res;
    };
    Log.serialize = function (data) {
        var attributeTypes = Log.getAttributeTypeMap();
        var res = {};
        for (var _i = 0, _a = Object.entries(data); _i < _a.length; _i++) {
            var _b = _a[_i], key = _b[0], value = _b[1];
            if (!(key in attributeTypes)) {
                throw new TypeError(key + " attribute not in schema");
            }
        }
        res.content = ObjectSerializer_1.ObjectSerializer.serialize(data.content, "LogContent", "");
        res.id = ObjectSerializer_1.ObjectSerializer.serialize(data.id, "string", "");
        return res;
    };
    Log.discriminator = undefined;
    Log.attributeTypeMap = {
        "content": {
            "baseName": "content",
            "type": "LogContent",
            "format": ""
        },
        "id": {
            "baseName": "id",
            "type": "string",
            "format": ""
        }
    };
    return Log;
}());
exports.Log = Log;
//# sourceMappingURL=Log.js.map
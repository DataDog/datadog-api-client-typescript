...
import { ChangeEventCreateRequestPayload } from "../models/ChangeEventCreateRequestPayload";
import { ChangeEventCreateResponsePayload } from "../models/ChangeEventCreateResponsePayload";
...
export class EventsApiRequestFactory extends BaseAPIRequestFactory {
  public async createEvent(
    ...
    logger.warn("Using unstable operation 'createEvent'");
    if (!_config.unstableOperations["v2.createEvent"]) {
      throw new Error("Unstable operation 'createEvent' is disabled");
    }
    ...
    const serializedBody = ObjectSerializer.stringify(
      ObjectSerializer.serialize(body, "ChangeEventCreateRequestPayload", ""),
      contentType
    );
...
export class EventsApiResponseProcessor {
  ...
  ): Promise<ChangeEventCreateResponsePayload> {
    const contentType = ObjectSerializer.normalizeMediaType(
      response.headers["content-type"]
    );
    if (response.httpStatusCode === 200) {
      const body: ChangeEventCreateResponsePayload =
        ObjectSerializer.deserialize(
          ObjectSerializer.parse(await response.body.text(), contentType),
          "ChangeEventCreateResponsePayload"
        ) as ChangeEventCreateResponsePayload;
      return body;
    }
...
export interface EventsApiCreateEventRequest {
   ...
   * @type ChangeEventCreateRequestPayload
   */
  body: ChangeEventCreateRequestPayload;
}

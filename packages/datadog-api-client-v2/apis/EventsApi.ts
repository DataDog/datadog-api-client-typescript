...
export class EventsApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async getEvent(eventId: string,_options?: Configuration): Promise<RequestContext> {
    ...
    // Path Params
    const localVarPath = '/api/v2/events/{event_id}'
      .replace('{event_id}', encodeURIComponent(String(eventId)));

    // Make Request Context
    const requestContext = _config.getServer('v2.EventsApi.getEvent').makeRequestContext(localVarPath, HttpMethod.GET);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);
...
export class EventsApi {
   ...
   * @param param The request object
   */
  public getEvent(param: EventsApiGetEventRequest, options?: Configuration): Promise<V2EventResponse> {

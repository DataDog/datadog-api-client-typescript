...
export class EventsApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async listOnCallEventEmailAddresses(
    filterTeamHandle: string,
    _options?: Configuration
  ): Promise<RequestContext> {
    const _config = _options || this.configuration;

    logger.warn("Using unstable operation 'listOnCallEventEmailAddresses'");
    if (!_config.unstableOperations["v2.listOnCallEventEmailAddresses"]) {
      throw new Error(
        "Unstable operation 'listOnCallEventEmailAddresses' is disabled"
      );
    }
    ...
    // Make Request Context
    const requestContext = _config
      .getServer("v2.EventsApi.listOnCallEventEmailAddresses")
      .makeRequestContext(localVarPath, HttpMethod.GET);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);
...
export class EventsApiResponseProcessor {
   ...
   * to the expected objects
   *
   * @params response Response returned by the server for a request to listOnCallEventEmailAddresses
   * @throws ApiException if the response code was not in [200, 299]
   */
  public async listOnCallEventEmailAddresses(
    response: ResponseContext
...
export interface EventsApiListOnCallEventEmailAddressesRequest {
  /**
...
export class EventsApi {
   ...
   * @param param The request object
   */
  public listOnCallEventEmailAddresses(
    param: EventsApiListOnCallEventEmailAddressesRequest,
    options?: Configuration
  ): Promise<EventEmailAddressesResponse> {
    const requestContextPromise =
      this.requestFactory.listOnCallEventEmailAddresses(
    ...
    return requestContextPromise.then((requestContext) => {
      return this.configuration.httpApi
        .send(requestContext)
        .then((responseContext) => {
          return this.responseProcessor.listOnCallEventEmailAddresses(
            responseContext
          );
        });
    });

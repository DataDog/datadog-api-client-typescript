...
import { DowntimeEditRequest } from "../models/DowntimeEditRequest";
...
export class DowntimesApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async updateDowntime(
    ...
    requestContext.setHeaderParam("Content-Type", contentType);
    const serializedBody = ObjectSerializer.stringify(
      ObjectSerializer.serialize(body, "DowntimeEditRequest", ""),
      contentType
    );
    requestContext.setBody(serializedBody);
...
export class DowntimesApiResponseProcessor {
  ...
  public async listMonitorDowntimes(
    response: ResponseContext
  ): Promise<Array<MonitorDowntimeMatchResponse>> {
    const contentType = ObjectSerializer.normalizeMediaType(
      response.headers["content-type"]
    );
    if (response.httpStatusCode == 200) {
      const body: Array<MonitorDowntimeMatchResponse> =
        ObjectSerializer.deserialize(
          ObjectSerializer.parse(await response.body.text(), contentType),
          "Array<MonitorDowntimeMatchResponse>"
        ) as Array<MonitorDowntimeMatchResponse>;
      return body;
    }

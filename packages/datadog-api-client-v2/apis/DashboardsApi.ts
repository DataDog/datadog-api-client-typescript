...
export class DashboardsApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async validateDashboardWidgets(
    ...
    logger.warn("Using unstable operation 'validateDashboardWidgets'");
    if (!_config.unstableOperations["v2.validateDashboardWidgets"]) {
      throw new Error(
        "Unstable operation 'validateDashboardWidgets' is disabled"
      );
    }
    ...
    // Make Request Context
    const requestContext = _config
      .getServer("v2.DashboardsApi.validateDashboardWidgets")
    ...
    const serializedBody = ObjectSerializer.stringify(
      ObjectSerializer.serialize(body, "DashboardWidgetValidationRequest", ""),
      contentType
    );
...
export class DashboardsApiResponseProcessor {
  ...
  public async validateDashboardWidgets(
    ...
    if (response.httpStatusCode === 200) {
      const body: DashboardWidgetValidationResponse =

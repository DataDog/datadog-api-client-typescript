...
export class DashboardSharingApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async listSharedDashboardsByDashboardId(dashboardId: string,_options?: Configuration): Promise<RequestContext> {
    ...
    if (!_config.unstableOperations['v2.listSharedDashboardsByDashboardId']) {
      throw new Error("Unstable operation 'listSharedDashboardsByDashboardId' is disabled");
    }
    ...
    // Path Params
    const localVarPath = '/api/v2/dashboard/{dashboard_id}/shared'
      .replace('{dashboard_id}', encodeURIComponent(String(dashboardId)));

    // Make Request Context
    const requestContext = _config.getServer('v2.DashboardSharingApi.listSharedDashboardsByDashboardId').makeRequestContext(localVarPath, HttpMethod.GET);
...
export class DashboardSharingApiResponseProcessor {
   ...
   */
   public async listSharedDashboardsByDashboardId(response: ResponseContext): Promise<ListSharedDashboardsResponse> {
...
export class DashboardSharingApi {
  ...
  public constructor(configuration: Configuration, requestFactory?: DashboardSharingApiRequestFactory, responseProcessor?: DashboardSharingApiResponseProcessor) {
    this.configuration = configuration;
    this.requestFactory = requestFactory || new DashboardSharingApiRequestFactory(configuration);
    this.responseProcessor = responseProcessor || new DashboardSharingApiResponseProcessor();
  }
   ...
   */
  public listSharedDashboardsByDashboardId(param: DashboardSharingApiListSharedDashboardsByDashboardIdRequest, options?: Configuration): Promise<ListSharedDashboardsResponse> {
    const requestContextPromise = this.requestFactory.listSharedDashboardsByDashboardId(param.dashboardId,options);

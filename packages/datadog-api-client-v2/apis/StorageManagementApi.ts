...
export class StorageManagementApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async deleteSyncConfig(id: string,_options?: Configuration): Promise<RequestContext> {
    ...
    // Path Params
    const localVarPath = '/api/v2/cloudinventoryservice/syncconfigs/{id}'
      .replace('{id}', encodeURIComponent(String(id)));

    // Make Request Context
    const requestContext = _config.getServer('v2.StorageManagementApi.deleteSyncConfig').makeRequestContext(localVarPath, HttpMethod.DELETE);
  ...
  public async upsertSyncConfig(body: UpsertCloudInventorySyncConfigRequest,_options?: Configuration): Promise<RequestContext> {
    const _config = _options || this.configuration;
    ...
    // Make Request Context
    const requestContext = _config.getServer('v2.StorageManagementApi.upsertSyncConfig').makeRequestContext(localVarPath, HttpMethod.PUT);
    ...
    const serializedBody = ObjectSerializer.stringify(
      ObjectSerializer.serialize(body, "UpsertCloudInventorySyncConfigRequest", ""),
      contentType
    );
...
export class StorageManagementApiResponseProcessor {
   ...
   * @throws ApiException if the response code was not in [200, 299]
   */
   public async upsertSyncConfig(response: ResponseContext): Promise<CloudInventorySyncConfigResponse> {
    const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 200) {
      const body: CloudInventorySyncConfigResponse = ObjectSerializer.deserialize(
...
export class StorageManagementApi {
  ...
  public constructor(configuration: Configuration, requestFactory?: StorageManagementApiRequestFactory, responseProcessor?: StorageManagementApiResponseProcessor) {
    this.configuration = configuration;
    this.requestFactory = requestFactory || new StorageManagementApiRequestFactory(configuration);
    this.responseProcessor = responseProcessor || new StorageManagementApiResponseProcessor();
  }
   ...
   */
  public deleteSyncConfig(param: StorageManagementApiDeleteSyncConfigRequest, options?: Configuration): Promise<void> {
   ...
   * @param param The request object
   */
  public upsertSyncConfig(param: StorageManagementApiUpsertSyncConfigRequest, options?: Configuration): Promise<CloudInventorySyncConfigResponse> {

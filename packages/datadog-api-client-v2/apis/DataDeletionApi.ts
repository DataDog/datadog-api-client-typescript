...
export class DataDeletionApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async cancelDataDeletionRequest(id: string,_options?: Configuration): Promise<RequestContext> {
    ...
    // Path Params
    const localVarPath = '/api/v2/deletion/requests/{id}/cancel'
      .replace('{id}', encodeURIComponent(String(id)));

    // Make Request Context
    const requestContext = _config.getServer('v2.DataDeletionApi.cancelDataDeletionRequest').makeRequestContext(localVarPath, HttpMethod.PUT);
  ...
  public async createDataDeletionRequest(product: string,body: CreateDataDeletionRequestBody,_options?: Configuration): Promise<RequestContext> {
    const _config = _options || this.configuration;
    ...
    // Path Params
    const localVarPath = '/api/v2/deletion/data/{product}'
      .replace('{product}', encodeURIComponent(String(product)));

    // Make Request Context
    const requestContext = _config.getServer('v2.DataDeletionApi.createDataDeletionRequest').makeRequestContext(localVarPath, HttpMethod.POST);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);
  ...
  public async getDataDeletionRequests(nextPage?: string,product?: string,query?: string,status?: string,pageSize?: number,_options?: Configuration): Promise<RequestContext> {
    const _config = _options || this.configuration;
    ...
    // Make Request Context
    const requestContext = _config.getServer('v2.DataDeletionApi.getDataDeletionRequests').makeRequestContext(localVarPath, HttpMethod.GET);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);

    // Query Params
    if (nextPage !== undefined) {
      requestContext.setQueryParam("next_page", ObjectSerializer.serialize(nextPage, "string", ""), "");
    }
...
export class DataDeletionApiResponseProcessor {
   ...
   */
   public async cancelDataDeletionRequest(response: ResponseContext): Promise<CancelDataDeletionResponseBody> {
    ...
    if (response.httpStatusCode === 400||response.httpStatusCode === 403||response.httpStatusCode === 412||response.httpStatusCode === 429||response.httpStatusCode === 500) {
   ...
   * @throws ApiException if the response code was not in [200, 299]
   */
   public async createDataDeletionRequest(response: ResponseContext): Promise<CreateDataDeletionResponseBody> {
    const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 200) {
   ...
   * @throws ApiException if the response code was not in [200, 299]
   */
   public async getDataDeletionRequests(response: ResponseContext): Promise<GetDataDeletionsResponseBody> {
    const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 200) {
    ...
    if (response.httpStatusCode === 400||response.httpStatusCode === 403||response.httpStatusCode === 429||response.httpStatusCode === 500) {
      const bodyText = ObjectSerializer.parse(await response.body.text(), contentType);
      let body: APIErrorResponse;
      try {
        body = ObjectSerializer.deserialize(
...
export class DataDeletionApi {
  ...
  public constructor(configuration: Configuration, requestFactory?: DataDeletionApiRequestFactory, responseProcessor?: DataDeletionApiResponseProcessor) {
    this.configuration = configuration;
    this.requestFactory = requestFactory || new DataDeletionApiRequestFactory(configuration);
    this.responseProcessor = responseProcessor || new DataDeletionApiResponseProcessor();
  }
   ...
   */
  public cancelDataDeletionRequest(param: DataDeletionApiCancelDataDeletionRequestRequest, options?: Configuration): Promise<CancelDataDeletionResponseBody> {
   ...
   * @param param The request object
   */
  public createDataDeletionRequest(param: DataDeletionApiCreateDataDeletionRequestRequest, options?: Configuration): Promise<CreateDataDeletionResponseBody> {
   ...
   * @param param The request object
   */
  public getDataDeletionRequests(param: DataDeletionApiGetDataDeletionRequestsRequest = {}, options?: Configuration): Promise<GetDataDeletionsResponseBody> {

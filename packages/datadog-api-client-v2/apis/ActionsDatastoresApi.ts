...
export class ActionsDatastoresApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async writeDatastoreItem(
    datastoreId: string,
    ...
    // verify required parameter 'datastoreId' is not null or undefined
    if (datastoreId === null || datastoreId === undefined) {
      throw new RequiredError("datastoreId", "writeDatastoreItem");
    }

    // verify required parameter 'body' is not null or undefined
    if (body === null || body === undefined) {
      throw new RequiredError("body", "writeDatastoreItem");
    }
    ...
    // Make Request Context
    const requestContext = _config
      .getServer("v2.ActionsDatastoresApi.writeDatastoreItem")
      .makeRequestContext(localVarPath, HttpMethod.POST);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);
...
export class ActionsDatastoresApiResponseProcessor {
   ...
   * to the expected objects
   *
   * @params response Response returned by the server for a request to writeDatastoreItem
   * @throws ApiException if the response code was not in [200, 299]
   */
  public async writeDatastoreItem(
    response: ResponseContext
...
export interface ActionsDatastoresApiWriteDatastoreItemRequest {
  /**
   * The ID of the datastore.
   * @type string
   */
...
export class ActionsDatastoresApi {
   ...
   * @param param The request object
   */
  public writeDatastoreItem(
    param: ActionsDatastoresApiWriteDatastoreItemRequest,
    options?: Configuration
  ): Promise<PutAppsDatastoreItemResponse> {
    const requestContextPromise = this.requestFactory.writeDatastoreItem(
      param.datastoreId,
      param.body,
      options
    );
    return requestContextPromise.then((requestContext) => {
      return this.configuration.httpApi
        .send(requestContext)
        .then((responseContext) => {
          return this.responseProcessor.writeDatastoreItem(responseContext);
        });
    });
  }
}

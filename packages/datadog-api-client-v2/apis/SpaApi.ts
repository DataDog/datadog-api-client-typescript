...
export class SpaApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async getSPARecommendations(service: string,bypassCache?: string,_options?: Configuration): Promise<RequestContext> {
    ...
    // Path Params
    const localVarPath = '/api/v2/spa/recommendations/{service}'
  ...
  if (bypassCache !== undefined) {
      requestContext.setQueryParam("bypass_cache", ObjectSerializer.serialize(bypassCache, "string", ""
), "");
    }
  ...
  public async getSPARecommendationsWithShard(shard: string,service: string,bypassCache?: string,_options?: Configuration): Promise<RequestContext> {
    const _config = _options || this.configuration;
    ...
    // Make Request Context
    const requestContext = _config.getServer('v2.SpaApi.getSPARecommendationsWithShard').makeRequestContext(localVarPath, HttpMethod.GET);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);
...
export class SpaApiResponseProcessor {
   ...
   * @throws ApiException if the response code was not in [200, 299]
   */
   public async getSPARecommendationsWithShard(response: ResponseContext): Promise<RecommendationDocument> {
    const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 200) {
      const body: RecommendationDocument = ObjectSerializer.deserialize(
        ObjectSerializer.parse(await response.body.text(), contentType),
...
export class SpaApi {
   ...
   * @param param The request object
   */
  public getSPARecommendationsWithShard(param: SpaApiGetSPARecommendationsWithShardRequest, options?: Configuration): Promise<RecommendationDocument> {

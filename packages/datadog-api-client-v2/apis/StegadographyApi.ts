...
export class StegadographyApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async getStegadographyWidgets(image: HttpFile,_options?: Configuration): Promise<RequestContext> {
    ...
    const requestContext = _config.getServer('v2.StegadographyApi.getStegadographyWidgets').makeRequestContext(localVarPath, HttpMethod.POST);
...
export class StegadographyApiResponseProcessor {
   ...
   */
   public async getStegadographyWidgets(response: ResponseContext): Promise<StegadographyGetWidgetsResponse> {
    const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 200) {
      const body: StegadographyGetWidgetsResponse = ObjectSerializer.deserialize(
        ObjectSerializer.parse(await response.body.text(), contentType),
        "StegadographyGetWidgetsResponse"
      ) as StegadographyGetWidgetsResponse;
      return body;
    }
    if (response.httpStatusCode === 400||response.httpStatusCode === 403||response.httpStatusCode === 415||response.httpStatusCode === 500) {
...
export class StegadographyApi {
  ...
  public constructor(configuration: Configuration, requestFactory?: StegadographyApiRequestFactory, responseProcessor?: StegadographyApiResponseProcessor) {
    this.configuration = configuration;
    this.requestFactory = requestFactory || new StegadographyApiRequestFactory(configuration);
    this.responseProcessor = responseProcessor || new StegadographyApiResponseProcessor();
  }
   ...
   */
  public getStegadographyWidgets(param: StegadographyApiGetStegadographyWidgetsRequest, options?: Configuration): Promise<StegadographyGetWidgetsResponse> {
    const requestContextPromise = this.requestFactory.getStegadographyWidgets(param.image,options);

...
export class AWSIntegrationApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async generateNewExternalID(
    _options?: Configuration
  ): Promise<RequestContext> {
    const _config = _options || this.configuration;

    logger.warn("Using unstable operation 'generateNewExternalID'");
    if (!_config.unstableOperations["v2.generateNewExternalID"]) {
      throw new Error("Unstable operation 'generateNewExternalID' is disabled");
    }
    ...
    // Make Request Context
    const requestContext = _config
      .getServer("v2.AWSIntegrationApi.generateNewExternalID")
      .makeRequestContext(localVarPath, HttpMethod.POST);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);
  ...
  public async listAWSLogServices(
    _options?: Configuration
  ): Promise<RequestContext> {
    const _config = _options || this.configuration;

    logger.warn("Using unstable operation 'listAWSLogServices'");
    if (!_config.unstableOperations["v2.listAWSLogServices"]) {
      throw new Error("Unstable operation 'listAWSLogServices' is disabled");
    }
    ...
    // Make Request Context
    const requestContext = _config
      .getServer("v2.AWSIntegrationApi.listAWSLogServices")
      .makeRequestContext(localVarPath, HttpMethod.GET);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);
  ...
  public async listAWSNamespaces(
    ...
    // Path Params
    const localVarPath = "/api/v2/integration/aws/namespaces";
...
export class AWSIntegrationApiResponseProcessor {
   ...
   * to the expected objects
   *
   * @params response Response returned by the server for a request to generateNewExternalID
   * @throws ApiException if the response code was not in [200, 299]
   */
  public async generateNewExternalID(
    response: ResponseContext
   ...
   * to the expected objects
   *
   * @params response Response returned by the server for a request to listAWSLogServices
   * @throws ApiException if the response code was not in [200, 299]
   */
  public async listAWSLogServices(
    response: ResponseContext
...
export class AWSIntegrationApi {
   ...
   * @param param The request object
   */
  public generateNewExternalID(
    options?: Configuration
  ): Promise<AWSNewExternalIDResponse> {
    const requestContextPromise =
      this.requestFactory.generateNewExternalID(options);
    return requestContextPromise.then((requestContext) => {
      return this.configuration.httpApi
        .send(requestContext)
        .then((responseContext) => {
          return this.responseProcessor.generateNewExternalID(responseContext);
        });
    });
  }
   ...
   * @param param The request object
   */
  public listAWSLogServices(
    options?: Configuration
  ): Promise<AWSLogsServicesResponse> {
    const requestContextPromise =
      this.requestFactory.listAWSLogServices(options);
    return requestContextPromise.then((requestContext) => {
      return this.configuration.httpApi
        .send(requestContext)
        .then((responseContext) => {
          return this.responseProcessor.listAWSLogServices(responseContext);
        });
    });
  }

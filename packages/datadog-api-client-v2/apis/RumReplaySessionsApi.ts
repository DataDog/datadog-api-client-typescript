...
export class RumReplaySessionsApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async getSegments(viewId: string,sessionId: string,source?: string,ts?: number,maxListSize?: number,paging?: string,_options?: Configuration): Promise<RequestContext> {
    ...
    const requestContext = _config.getServer('v2.RumReplaySessionsApi.getSegments').makeRequestContext(localVarPath, HttpMethod.GET);
  ...
  if (maxListSize !== undefined) {
      requestContext.setQueryParam("max_list_size", ObjectSerializer.serialize(maxListSize, "number", "int64"
), "");
    }
  if (paging !== undefined) {
      requestContext.setQueryParam("paging", ObjectSerializer.serialize(paging, "string", ""
), "");
    }
...
export class RumReplaySessionsApi {
  ...
  public constructor(configuration: Configuration, requestFactory?: RumReplaySessionsApiRequestFactory, responseProcessor?: RumReplaySessionsApiResponseProcessor) {
    this.configuration = configuration;
    this.requestFactory = requestFactory || new RumReplaySessionsApiRequestFactory(configuration);
    this.responseProcessor = responseProcessor || new RumReplaySessionsApiResponseProcessor();
  }
   ...
   */
  public getSegments(param: RumReplaySessionsApiGetSegmentsRequest, options?: Configuration): Promise<void> {

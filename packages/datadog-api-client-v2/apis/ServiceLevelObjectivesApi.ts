...
export class ServiceLevelObjectivesApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async getSloStatus(sloId: string,fromTs: number,toTs: number,disableCorrections?: boolean,_options?: Configuration): Promise<RequestContext> {
    const _config = _options || this.configuration;
    ...
    // Path Params
    const localVarPath = '/api/v2/slo/{slo_id}/status'
      .replace('{slo_id}', encodeURIComponent(String(sloId)));

    // Make Request Context
    const requestContext = _config.getServer('v2.ServiceLevelObjectivesApi.getSloStatus').makeRequestContext(localVarPath, HttpMethod.GET);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);
  ...
  if (disableCorrections !== undefined) {
      requestContext.setQueryParam("disable_corrections", ObjectSerializer.serialize(disableCorrections, "boolean", ""
), "");
    }
...
export class ServiceLevelObjectivesApiResponseProcessor {
   ...
   * @throws ApiException if the response code was not in [200, 299]
   */
   public async getSloStatus(response: ResponseContext): Promise<SloStatusResponse> {
    const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 200) {
...
export class ServiceLevelObjectivesApi {
   ...
   * @param param The request object
   */
  public getSloStatus(param: ServiceLevelObjectivesApiGetSloStatusRequest, options?: Configuration): Promise<SloStatusResponse> {

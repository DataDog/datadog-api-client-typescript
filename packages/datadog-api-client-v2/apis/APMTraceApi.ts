...
export class APMTraceApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async getPrunedTraceByID(traceId: string,expandSpanId?: number,timeHint?: number,forceSource?: string,includePath?: Array<string>,tagInclude?: Array<string>,tagExclude?: Array<string>,onlyServiceEntrySpans?: boolean,_options?: Configuration): Promise<RequestContext> {
    ...
    // Path Params
    const localVarPath = '/api/v2/pruned_trace/{trace_id}'
      .replace('{trace_id}', encodeURIComponent(String(traceId)));

    // Make Request Context
    const requestContext = _config.getServer('v2.APMTraceApi.getPrunedTraceByID').makeRequestContext(localVarPath, HttpMethod.GET);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);

    // Query Params
  if (expandSpanId !== undefined) {
      requestContext.setQueryParam("expand_span_id", ObjectSerializer.serialize(expandSpanId, "number", "int64"
), "");
    }
  if (timeHint !== undefined) {
      requestContext.setQueryParam("time_hint", ObjectSerializer.serialize(timeHint, "number", "int32"
), "");
    }
  if (forceSource !== undefined) {
      requestContext.setQueryParam("force_source", ObjectSerializer.serialize(forceSource, "string", ""
), "");
    }
  if (includePath !== undefined) {
      requestContext.setQueryParam("include_path", ObjectSerializer.serialize(includePath, "Array<string>", ""
), "multi");
    }
  if (tagInclude !== undefined) {
      requestContext.setQueryParam("tag_include", ObjectSerializer.serialize(tagInclude, "Array<string>", ""
), "multi");
    }
  if (tagExclude !== undefined) {
      requestContext.setQueryParam("tag_exclude", ObjectSerializer.serialize(tagExclude, "Array<string>", ""
), "multi");
    }
  if (onlyServiceEntrySpans !== undefined) {
      requestContext.setQueryParam("only_service_entry_spans", ObjectSerializer.serialize(onlyServiceEntrySpans, "boolean", ""
), "");
    }
  ...
  public async getTraceByID(traceId: string,includeFields?: Array<string>,_options?: Configuration): Promise<RequestContext> {
    const _config = _options || this.configuration;
    ...
    // Path Params
    const localVarPath = '/api/v2/trace/{trace_id}'
      .replace('{trace_id}', encodeURIComponent(String(traceId)));

    // Make Request Context
    const requestContext = _config.getServer('v2.APMTraceApi.getTraceByID').makeRequestContext(localVarPath, HttpMethod.GET);
    requestContext.setHeaderParam("Accept", "application/json");
    requestContext.setHttpConfig(_config.httpConfig);

    // Query Params
  if (includeFields !== undefined) {
      requestContext.setQueryParam("include_fields", ObjectSerializer.serialize(includeFields, "Array<string>", ""
), "multi");
    }
...
export class APMTraceApiResponseProcessor {
   ...
   */
   public async getPrunedTraceByID(response: ResponseContext): Promise<PrunedTraceResponse> {
    ...
    if (response.httpStatusCode === 403||response.httpStatusCode === 404||response.httpStatusCode === 504) {
   ...
   */
   public async getTraceByID(response: ResponseContext): Promise<TraceResponse> {
    ...
    if (response.httpStatusCode === 403||response.httpStatusCode === 404||response.httpStatusCode === 413) {
      const bodyText = ObjectSerializer.parse(await response.body.text(), contentType);
      let body: JSONAPIErrorResponse;
      try {
        body = ObjectSerializer.deserialize(
...
export class APMTraceApi {
  ...
  public constructor(configuration: Configuration, requestFactory?: APMTraceApiRequestFactory, responseProcessor?: APMTraceApiResponseProcessor) {
    this.configuration = configuration;
    this.requestFactory = requestFactory || new APMTraceApiRequestFactory(configuration);
    this.responseProcessor = responseProcessor || new APMTraceApiResponseProcessor();
  }
   ...
   */
  public getPrunedTraceByID(param: APMTraceApiGetPrunedTraceByIDRequest, options?: Configuration): Promise<PrunedTraceResponse> {
   ...
   * @param param The request object
   */
  public getTraceByID(param: APMTraceApiGetTraceByIDRequest, options?: Configuration): Promise<TraceResponse> {

...
Feature: Workflow Automation
  ...
  Scenario: Generate workflow description returns "Bad Request" response
    ...
    And body with value {"name": "Alert Response Workflow", "spec": {"annotations": [{"display": {"bounds": {}}, "id": "", "markdownTextAnnotation": {}}], "connectionEnvs": [{"connectionGroups": [{"connectionGroupId": "", "label": "", "tags": [""]}], "connections": [{"connectionId": "", "label": ""}], "env": "default"}], "inputSchema": {"parameters": [{"name": "", "type": "STRING"}]}, "outputSchema": {"parameters": [{"name": "", "type": "STRING"}]}, "steps": [{"actionId": "", "completionGate": {"completionCondition": {"operator": "OPERATOR_EQUAL"}, "retryStrategy": {"kind": "RETRY_STRATEGY_LINEAR", "linear": {"interval": "", "maxRetries": 0.0}}}, "display": {"bounds": {}}, "errorHandlers": [{"fallbackStepName": "", "retryStrategy": {"kind": "RETRY_STRATEGY_LINEAR", "linear": {"interval": "", "maxRetries": 0.0}}}], "name": "", "outboundEdges": [{"branchName": "", "nextStepName": ""}], "parameters": [{"name": ""}], "readinessGate": {"thresholdType": "ANY"}}], "triggers": [{"apiTrigger": {"rateLimit": {}}, "startStepNames": [""]}]}}
    When the request is sent
    Then the response status is 400 Bad Request

...
Feature: Agentless Scanning
  ...
  Scenario: Delete AWS Scan Options returns "OK" response
    Given new "DeleteAwsScanOptions" request
  ...
  @team:DataDog/k9-cloud-security-platform
  Scenario: Patch AWS Scan Options returns "OK" response
    Given new "UpdateAwsScanOptions" request
    And request contains "account_id" parameter with value "000000000002"
    And body with value {"data":{"type":"aws_scan_options","id":"000000000002","attributes":{"vuln_host_os":true,"vuln_containers_os":true}}}
    When the request is sent
    Then the response status is 204 OK
  ...
  Scenario: Post AWS Scan Options returns "Agentless scan options enabled successfully." response
    ...
    And body with value {"data":{"type":"aws_scan_options","id":"000000000003","attributes":{"vuln_host_os":true,"vuln_containers_os":true,"lambda":false}}}
    When the request is sent
  ...
  Scenario: Post AWS Scan Options returns "Bad Request" response
    Given new "CreateAwsScanOptions" request
    And body with value {"data": {"id": "184366314700", "type": "incorrect_type", "attributes": {"lambda": true, "sensitive_data": false, "vuln_containers_os": true, "vuln_host_os": true}}}
    When the request is sent
    Then the response status is 400 Bad Request

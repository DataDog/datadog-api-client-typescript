...
Feature: Events
  ...
  @generated @skip @team:DataDog/event-management
  Scenario: Get on-call event email addresses returns "Bad Request" response
    Given operation "GetOnCallEventEmailAddresses" enabled
    And new "GetOnCallEventEmailAddresses" request
    And request contains "filter[team_handle]" parameter from "REPLACE.ME"
    When the request is sent
    Then the response status is 400 Bad Request

  @generated @skip @team:DataDog/event-management
  Scenario: Get on-call event email addresses returns "OK" response
    Given operation "GetOnCallEventEmailAddresses" enabled
    And new "GetOnCallEventEmailAddresses" request
    And request contains "filter[team_handle]" parameter from "REPLACE.ME"
    When the request is sent

...
export class DelegatedTokenApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public constructor(configuration: Configuration) {
    super(configuration);
    if (!isBrowser) {
      this.userAgent = buildUserAgent("delegated-token", version);
    }
  }
  public async getDelegatedToken(
    ...
    const { server, overrides } = _config.getServerAndOverrides(
      ...
      DelegatedTokenApi.operationServers,
    );
...
export class DelegatedTokenApiResponseProcessor {
  ...
  public async getDelegatedToken(
    ...
    if (response.httpStatusCode === 200) {
      const body: DelegatedTokenResponse = deserialize(
...
export class DelegatedTokenApi {
  ...
  public constructor(
    ...
    this.requestFactory =
      requestFactory || new DelegatedTokenApiRequestFactory(this.configuration);

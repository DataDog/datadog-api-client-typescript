# CHANGELOG

## 0.0.1-beta.3/2026-06-30

### Added
* Add OpenAPI spec for APM spans-public-api trace endpoints [#4298](https://github.com/DataDog/datadog-api-client-typescript/pull/4298)

## 0.0.1-beta.2/2026-02-17

### Fixed
* Add mandatory filter env parameter to APM services endpoint [#3476](https://github.com/DataDog/datadog-api-client-typescript/pull/3476)

## 0.0.1-beta.1/2025-12-17

### Security
* Bump the `datadog-api-client` min version [#3129](https://github.com/DataDog/datadog-api-client-typescript/pull/3129)

### Added
* Add `GET /api/v2/apm/services` endpoint to public documentation [#3098](https://github.com/DataDog/datadog-api-client-typescript/pull/3098)

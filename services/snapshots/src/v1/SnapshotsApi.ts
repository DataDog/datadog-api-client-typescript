...
export class SnapshotsApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public constructor(configuration: Configuration) {
    super(configuration);
    if (!isBrowser) {
      this.userAgent = buildUserAgent("snapshots", version);
    }
  }
  public async getGraphSnapshot(
    ...
    const { server, overrides } = _config.getServerAndOverrides("SnapshotsApi.v1.getGraphSnapshot", SnapshotsApi.operationServers);

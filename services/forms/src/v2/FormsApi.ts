...
export class FormsApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async listFormVersions(
    ...
    if (!_config.unstableOperations["FormsApi.v2.listFormVersions"]) {
      throw new Error(
        "Unstable operation 'listFormVersions' is disabled. Enable it by setting `configuration.unstableOperations['FormsApi.v2.listFormVersions'] = true`",
      );
    }
  ...
  public async revertFormVersion(
    ...
    if (!_config.unstableOperations["FormsApi.v2.revertFormVersion"]) {
      throw new Error(
        "Unstable operation 'revertFormVersion' is disabled. Enable it by setting `configuration.unstableOperations['FormsApi.v2.revertFormVersion'] = true`",
      );
    }
...
export class FormsApiResponseProcessor {
  ...
  public async listFormVersions(
    ...
    const contentType = normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 200) {
      const body: ListFormVersionsResponse = deserialize(
        parse(await response.body.text(), contentType),
        TypingInfo,

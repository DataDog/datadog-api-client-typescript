...
export class ProductCatalogApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public constructor(configuration: Configuration) {
    super(configuration);
    if (!isBrowser) {
      this.userAgent = buildUserAgent("product-catalog", version);
    }
  }
  public async listProductCatalogSKUs(
    ...
    if (
      !_config.unstableOperations["ProductCatalogApi.v2.listProductCatalogSKUs"]
    ) {
      throw new Error(
        "Unstable operation 'listProductCatalogSKUs' is disabled. Enable it by setting `configuration.unstableOperations['ProductCatalogApi.v2.listProductCatalogSKUs'] = true`",
      );
    }
    ...
    const { server, overrides } = _config.getServerAndOverrides(
      ...
      ProductCatalogApi.operationServers,
    );
    ...
    if (version !== undefined) {
      requestContext.setQueryParam(
        "version",
        serialize(version, TypingInfo, "ProductCatalogSKUsAPIVersion", ""),
        "",
      );
    }
    if (asOfDate !== undefined) {
      requestContext.setQueryParam(
        "as_of_date",
        serialize(asOfDate, TypingInfo, "Date", "date"),
        "",
      );
    }
...
export class ProductCatalogApiResponseProcessor {
  ...
  public async listProductCatalogSKUs(
    ...
    if (response.httpStatusCode === 200) {
      const body: ProductCatalogSKUsResponse = deserialize(
...
export class ProductCatalogApi {
  ...
  public constructor(
    ...
    this.requestFactory =
      requestFactory || new ProductCatalogApiRequestFactory(this.configuration);

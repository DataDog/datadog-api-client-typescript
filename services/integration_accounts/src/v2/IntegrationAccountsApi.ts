...
export class IntegrationAccountsApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public async createAmsIntegrationAccount(
    ...
    const serializedBody = stringify(
      serialize(body, TypingInfo, "AmsIntegrationAccountCreateRequest", ""),
      contentType,
    );
  ...
  public async updateAmsIntegrationAccount(
    ...
    requestContext.setHeaderParam("Content-Type", contentType);
    const serializedBody = stringify(
      serialize(body, TypingInfo, "AmsIntegrationAccountUpdateRequest", ""),
      contentType,
    );
    requestContext.setBody(serializedBody);
...
export class IntegrationAccountsApiResponseProcessor {
  ...
  public async createAmsIntegrationAccount(
    ...
    if (response.httpStatusCode === 201) {
      const body: AmsIntegrationAccountResponse = deserialize(
  ...
  public async getAmsIntegrationAccountSchema(
    ...
    const contentType = normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 200) {
      const body: AmsIntegrationAccountSchemaResponse = deserialize(
        parse(await response.body.text(), contentType),
        TypingInfo,
  ...
  public async listAmsIntegrationAccounts(
    ...
    const contentType = normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 200) {
      const body: AmsIntegrationAccountsResponse = deserialize(
        parse(await response.body.text(), contentType),
        TypingInfo,

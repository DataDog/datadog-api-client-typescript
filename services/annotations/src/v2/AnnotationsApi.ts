...
export class AnnotationsApiRequestFactory extends BaseAPIRequestFactory {
  ...
  public constructor(configuration: Configuration) {
    super(configuration);
    if (!isBrowser) {
      this.userAgent = buildUserAgent("annotations", version);
    }
  }
  ...
  public async getPageAnnotations(
    ...
    if (startTime !== undefined) {
      requestContext.setQueryParam(
        "start_time",
        serialize(startTime, TypingInfo, "number", "int64"),
        "",
      );
    }
    if (endTime !== undefined) {
      requestContext.setQueryParam(
        "end_time",
        serialize(endTime, TypingInfo, "number", "int64"),
        "",
      );
    }
  ...
  public async listAnnotations(
    ...
    if (pageId !== undefined) {
      requestContext.setQueryParam(
        "page_id",
        serialize(pageId, TypingInfo, "string", ""),
        "",
      );
    }
    if (startTime !== undefined) {
    ...
    if (widgetId !== undefined) {
      requestContext.setQueryParam(
        "widget_id",
        serialize(widgetId, TypingInfo, "string", ""),
        "",
      );
    }

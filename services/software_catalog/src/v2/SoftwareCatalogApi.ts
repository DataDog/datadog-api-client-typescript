...
import { AcceptRecommendedEntitiesResponse } from "./models/AcceptRecommendedEntitiesResponse";
...
export class SoftwareCatalogApiResponseProcessor {
  ...
  ): Promise<AcceptRecommendedEntitiesResponse> {
    const contentType = normalizeMediaType(response.headers["content-type"]);
    if (response.httpStatusCode === 202) {
      const body: AcceptRecommendedEntitiesResponse = deserialize(
        parse(await response.body.text(), contentType),
        TypingInfo,
        "AcceptRecommendedEntitiesResponse",
      ) as AcceptRecommendedEntitiesResponse;
      return body;
    }

# CHANGELOG

## 0.0.1-beta.7/2026-09-01

### Added
* Add is_iac support to send X-Datadog-Managed-By header [#4945](https://github.com/DataDog/datadog-api-client-typescript/pull/4945)

## 0.0.1-beta.6/2026-06-17

### Added
* [error-tracking] Add Linear issue to API endpoints response schemas [#4383](https://github.com/DataDog/datadog-api-client-typescript/pull/4383)

## 0.0.1-beta.5/2026-05-15

### Added
* Add regression fields to Error Tracking API responses [#4133](https://github.com/DataDog/datadog-api-client-typescript/pull/4133)
* Add state filter for Search Error Tracking Issues API [#4088](https://github.com/DataDog/datadog-api-client-typescript/pull/4088)

## 0.0.1-beta.4/2026-04-16

### Added
* Add team and assignee filters for Search Error Tracking Issues API [#3912](https://github.com/DataDog/datadog-api-client-typescript/pull/3912)

## 0.0.1-beta.3/2026-04-16

### Added
* Add team and assignee filters for Search Error Tracking Issues API [#3912](https://github.com/DataDog/datadog-api-client-typescript/pull/3912)

## 0.0.1-beta.2/2026-01-08

### Added
* Update /api/v2/cases endpoint to add custom attributes support [#3202](https://github.com/DataDog/datadog-api-client-typescript/pull/3202)

## 0.0.1-beta.1/2025-12-17

### Security
* Bump the `datadog-api-client` min version [#3129](https://github.com/DataDog/datadog-api-client-typescript/pull/3129)

...
v2ApiInstance
  ...
  .then((fieldsData: v2.UsageSummaryAvailableFieldsResponse) => {
    ...
    const params: v1.UsageMeteringApiGetUsageSummaryRequest = {
      startMonth: new Date(2026, 4, 1, 0, 0, 0, 0), // May 2026
      endMonth: new Date(2026, 4, 1, 0, 0, 0, 0), // May 2026
    ...
    v1ApiInstance
      ...
      .then((data: v1.UsageSummaryResponse) => {
        ...
        for (const dateEntry of usageDates) {
          ...
          // --- per-date per-org entries ---
          const orgs = dateEntry.orgs ?? [];
          for (const orgEntry of orgs) {
            const orgName =
              (orgEntry as any).name ?? (orgEntry as any).publicId ?? "(org)";
            console.log(`\n  === Org: ${orgName} ===`);

            const orgProps =
              (orgEntry as v1.UsageSummaryDateOrg).additionalProperties ?? {};

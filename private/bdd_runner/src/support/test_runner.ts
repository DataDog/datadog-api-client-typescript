...
BeforeAll(async function () {
  ...
  const logPath =
    process.env.DD_TEST_SERVER_LOG || "/tmp/datadog-typescript-test-server.log";
  serverLog = openSync(logPath, "w");
  serverProcess = spawn(executable, ["--port", port], {
...
export function testRunnerEnabled(): boolean {
  return (
    ...
    process.env.DD_TEST_RUNNER_DATA !== undefined
  );
}
...
export function testServerEnabled(): boolean {
  return (
    (process.env.DD_USE_GENERATED_TESTS || "false").toLowerCase() === "true" &&
    process.env.DD_TEST_SERVER_URL !== undefined
  );
}
...
export function applyTestRunnerPlan(world: World, pagination: boolean): void {
  ...
  world.apiName = `${plan.api.replace(/-/g, "")}Api`;
